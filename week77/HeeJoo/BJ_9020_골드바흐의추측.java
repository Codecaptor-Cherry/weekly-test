...
public class BJ_9020_골드바흐의추측 {
    public static void main(String[] args) throws IOException {
        ...
        for(int i = 0; i < T; i++) {
            inputs[i] = stoi(br.readLine()); // 짝수 n
        ...
        for(int i = 0; i < T; i++) {
            for(int j = inputs[i] / 2; j > 1; j--) { // 두 소수의 차이가 가장 작은 것을 출력하니 중간부터 거꾸로 확인
                if(prime[j] && prime[inputs[i] - j]) {
                    sb.append(j + " " + (inputs[i] - j) + "\n");
                    break;
                }
            }
        }
    ...
    public static void eratos(int k, boolean[] prime) {
        for(int i = 2; i <= Math.sqrt(k); i++) { // 3. 입력값 k의 제곱근까지 확인
